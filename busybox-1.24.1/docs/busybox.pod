...
Giving controlling tty to shell running with PID 1:
	$ exec cttyhack sh
Starting interactive shell from boot shell script:n
	setsid cttyhack sh
...
It also can be ran from tcpsvd:n
...
Device table entries take the form of:n
...
to load modules for hotplugged devices:n
...
SELECTOR can be a combination of:n
	driver=STRING
	bus=STRING
	phy_address=NUM
...
Set soft resource limits, then run PROG
	...
	-o N		Limit number of open files per process
	-p N		Limit number of processes per uid
Options controlling file sizes:n
	-f BYTES	Limit output file sizes
	-c BYTES	Limit core file size
Efficiency opts:n
...
Valid format sequences for files:n
...
Valid format sequences for file systems:n
...
Free initramfs and switch to another root fs:n
...
It also can be ran from udpsvd:n
